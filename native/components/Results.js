...
export default class Results extends React.Component {
  ...
  renderVirusSVG(color) {
    return (
      <Svg xmlns="http://www.w3.org/2000/svg" version="1.0" width="100%" height="900.000000pt" viewBox="0 0 900.000000 900.000000" preserveAspectRatio="xMidYMid meet">
  ...
  renderViruses(heightPercent) {
    ...
    for (const virus of this.state.viruses) {
      let offset = (100 - (100 * calculateLogFraction(this.props.gsv)));
      const virusStyle = {
        // bottom: `${100 + (100 * calculateLogFraction(this.props.gsv)/2) + virus.lat}%`,
        bottom: `${virus.lat}%`,
        left: `${virus.lng}%`,
        width: virus.size,
        height: virus.size,
        ...
        transform: [{translateX: -virus.size/2}],
        zIndex: 100,
      }
      viruses.push(
        ...
        // <Image source={virusIcon} style={virusStyle} key={virus.id} />
      );
    }
    return viruses;
  }
